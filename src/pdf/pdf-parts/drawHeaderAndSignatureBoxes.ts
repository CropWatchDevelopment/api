...
export function drawHeaderAndSignatureBoxes(
  ...
  doc.fontSize(14).text(`レポート (${timeSpan})`, startX, startY, { width: headerTableWidth });
...
function formatDateForRange(date) {
  const dateToDisplay = moment(moment(date).tz('Asia/Tokyo').toDate()).subtract(9, 'h').format('YYYY-MM-DD HH:mm').toString();
