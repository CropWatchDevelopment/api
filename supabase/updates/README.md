...
| `008_DESTRUCTIVE_legacy_table_drops.sql` | Legacy table drops (**applied** — the dropped tables are gone from production; `database.types.ts` still lists some of them) | Last, after everything is stable; take a backup first |
...
| `022_push_tokens.sql` | Creates `cw_push_tokens` (FCM web-push token registry) | Before deploying the push-notification API release |
| `023_push_action_type.sql` | Seeds `cw_rule_action_types` with the Push action — data-driven rules-UI option | **Last** — only after the alert service handles push actions |
| `024_billing_seats_v2.sql` | Billing v2: `billing_customers.billing_mode` (`stripe`/`manual`), `reporting_subscription_id`/`reporting_status`/`reporting_manual`; `device_licenses.stripe_subscription_id` nullable (NULL = staff-granted seat); license FK `ON UPDATE CASCADE`. Additive. Footer holds a one-off OPS block that clears the test-mode Stripe ids that leaked into prod | Main block **before** deploying the billing-v2 API release; OPS block after. Patch `database.types.ts` (api + CropWatch) after |
| `025_organizations.sql` | Organizations v1 (additive): `organizations` / `organization_members` / `organization_invites` / `organization_link_requests`, `org_id` on locations/devices/billing/licenses/report-templates, `is_legacy` markers, the missing `UNIQUE(location_id,user_id)` on `cw_location_owners` (dedupes first), invariant + guest-cap + org-fill compatibility triggers, `handle_new_user()` home-org creation, and the org RPCs (`accept_org_invite`, `remove_org_member`, `accept_org_link`, `unlink_org`, `convert_org_to_company`, `transfer_org_ownership`). Single-org membership model: one active full membership per person, enforced in-trigger. Run the commented PREFLIGHT block first; verify with the OPS footer | **After the PR-A API release is live** (it is). Before deploying the org-aware API (PR-B). Regenerate `database.types.ts` (api + CropWatch) after |
| `026_gateways_org.sql` | Gateways become org-owned: `cw_gateways.org_id` + backfill from the earliest `cw_gateways_owners` row + a first-owner claim trigger (an owner row inserted for an unclaimed gateway claims it for that user's active org). Split out of 025 after 025 had run | **After 025. Before deploying the org-aware API (PR-B)** — its gateway scoping reads `org_id`. Patch `database.types.ts` after |
